...
export default function ParametersPage() {
  const [data, setData] = useState<Center>(empty); const [tab, setTab] = useState<Tab>("general"); const [message, setMessage] = useState(""); const [query, setQuery] = useState("");
  ...
  return <Shell title="Centro de configuración" subtitle="Todas las parametrizaciones funcionales, de acceso e integración en un solo lugar.">
    ...
    {tab === "integrations" && <div className="config-columns two"><Panel title="Microsoft 365 y Power Platform"><div className="integration-list">{["Microsoft Graph","Outlook","Teams","Planner","Power BI","Power Automate"].map(name => <div key={name}><span className="integration-icon">↗</span><div><strong>{name}</strong><small>Pendiente de credenciales institucionales</small></div><span className="tag high">Por configurar</span></div>)}</div></Panel><Panel title="Notificaciones y acceso público"><div className="parameter-list">{data.parameters.filter(item => ["PUBLIC","NOTIFICATIONS","INTEGRATIONS"].includes(item.group)).map(item => <div key={item.id}><div><strong>{item.description}</strong><small>{item.group} · {item.key}</small></div><input value={item.value} onChange={event => update(item.id, event.target.value)} /><button className="button ghost" onClick={() => save(item)}>Guardar</button></div>)}</div><div className="empty compact">Los secretos se administran por ambiente y nunca se muestran aquí.</div></Panel></div>}
  </Shell>;
}
