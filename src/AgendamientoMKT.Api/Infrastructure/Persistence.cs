...
public sealed class AdministrationRepository(AppDbContext db) : IAdministrationRepository
{
    ...
    public async Task<IReadOnlyCollection<RoleConfigDto>> RoleConfigurationAsync(CancellationToken ct) => await db.Roles.AsNoTracking().Include(x => x.Permissions).ThenInclude(x => x.Permission).OrderBy(x => x.Name).Select(x => new RoleConfigDto(x.Id, x.Code, x.Name, x.IsActive, x.Permissions.Select(p => p.Permission.Code).OrderBy(code => code).ToArray())).ToArrayAsync(ct);
    public async Task<IReadOnlyCollection<ScreenDto>> ScreensAsync(CancellationToken ct) => await db.Screens.AsNoTracking().OrderBy(x => x.Name).Select(x => new ScreenDto(x.Id, x.Code, x.Name, x.Route, x.IsActive)).ToArrayAsync(ct);
    public async Task<IReadOnlyCollection<MenuDto>> AllMenuAsync(CancellationToken ct) => await db.MenuItems.AsNoTracking().OrderBy(x => x.Order).Select(x => new MenuDto(x.Code, x.Label, x.Route, x.Order, x.RequiredPermission)).ToArrayAsync(ct);
}
